...
class StoryRepositoryImpl: StoryRepository {
    ...
    val ref: DatabaseReference = database.reference.child("stories")
    override fun addStory(storyModel: StoryModel, callback: (Boolean, String) -> Unit) {

        var id = ref.push().key.toString()
        storyModel.storyId = id
        ref.child(id).setValue(storyModel).addOnCompleteListener {
            if(it.isSuccessful){
    ...
    override fun updateStory(
        ...
        ref.child(storyId).updateChildren(data).addOnCompleteListener {
            if(it.isSuccessful){
    ...
    override fun deleteStory(storyId: String, callback: (Boolean, String) -> Unit) {
        ref.child(storyId).removeValue().addOnCompleteListener {
            if(it.isSuccessful){
    ...
    override fun getStoryById(
        storyId: String,
        callback: (Boolean, String) -> Unit) {
        ref.child(storyId).addValueEventListener(
                ...
                override fun onDataChange(snapshot: DataSnapshot) {
                    ...
                    if (snapshot.exists()){
                        ...
                        for(eachData in snapshot.children){
                            var model = eachData.getValue(StoryModel::class.java)
                            if (model != null){
                                stories.add(model)
                            }
                        }
                        callback(stories,true,"Stories fetched")
                    }
                }
